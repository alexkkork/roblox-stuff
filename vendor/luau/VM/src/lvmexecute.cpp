...
static lua_TraceValue luau_tracevalue(const TValue* value)
{
    lua_TraceValue result = {};
    if (!value)
    {
        result.type = LUA_TNONE;
        return result;
    }

    result.type = ttype(value);
    if (ttisboolean(value))
        result.booleanValue = bvalue(value);
    else if (ttisnumber(value))
        result.numberValue = nvalue(value);
    else if (ttisinteger(value))
        result.numberValue = double(lvalue(value));
    else if (ttisstring(value))
    {
        result.stringValue = getstr(tsvalue(value));
        result.stringLength = tsvalue(value)->len;
    }
    else if (iscollectable(value))
        result.pointerValue = gcvalue(value);
    else if (ttislightuserdata(value))
        result.pointerValue = pvalue(value);
    return result;
}

static void luau_traceaccess(lua_State* L, int kind, LuaTable* table, const TValue* key, const TValue* value)
{
    void (*callback)(lua_State*, const lua_TraceAccess*) = L->global->cb.traceaccess;
    if (!callback)
        return;

    lua_TraceAccess access = {};
    access.kind = kind;
    access.tablePointer = table;
    access.key = luau_tracevalue(key);
    access.value = luau_tracevalue(value);
    callback(L, &access);
}
...
#endif
        {
            ...
            VM_CASE(LOP_GETGLOBAL)
            {
                ...
                if (LUAU_LIKELY(ttisstring(gkey(n)) && tsvalue(gkey(n)) == tsvalue(kv)) && !ttisnil(gval(n)))
                {
                    ...
                    luau_traceaccess(L, LUA_TRACE_GET_GLOBAL, h, kv, VM_REG(LUAU_INSN_A(insn)));
                    VM_NEXT();
                }
                else
                {
            ...
            VM_CASE(LOP_SETGLOBAL)
            {
                ...
                TValue* kv = VM_KV(aux);
                LUAU_ASSERT(ttisstring(kv));
                luau_traceaccess(L, LUA_TRACE_SET_GLOBAL, cl->env, kv, ra);
            ...
            VM_CASE(LOP_GETIMPORT)
            {
                ...
                VM_CASE_STKID ra = VM_REG(LUAU_INSN_A(insn));
                TValue* kv = VM_KV(LUAU_INSN_D(insn));
                uint32_t importAux = pc[0];
                TValue* importKey = VM_KV(int(importAux >> 20) & 1023);
                ...
                if (!ttisnil(kv) && cl->env->safeenv)
                {
                    setobj2s(L, ra, kv);
                    pc++; // skip over AUX
                    luau_traceaccess(L, LUA_TRACE_GET_IMPORT, cl->env, importKey, VM_REG(LUAU_INSN_A(insn)));
                    VM_NEXT();
                }
                else
                {
            ...
            VM_CASE(LOP_GETTABLEKS)
            {
                ...
                if (LUAU_LIKELY(ttistable(rb)))
                {
                    ...
                    const bool traceEnvironment = h == cl->env || h == L->global->cb.tracesharedglobal;
                    ...
                    if (LUAU_LIKELY(ttisstring(gkey(n)) && tsvalue(gkey(n)) == tsvalue(kv) && !ttisnil(gval(n))))
                    {
                        setobj2s(L, ra, gval(n));
                        if (traceEnvironment)
                            luau_traceaccess(L, LUA_TRACE_GET_TABLE, h, kv, VM_REG(LUAU_INSN_A(insn)));
                        VM_NEXT();
                    }
                    else if (!h->metatable)
                    {
                        ...
                        setobj2s(L, ra, res);
                        if (traceEnvironment)
                            luau_traceaccess(L, LUA_TRACE_GET_TABLE, h, kv, ra);
                        VM_NEXT();
                    }
                    else
                    {
            ...
            VM_CASE(LOP_SETTABLEKS)
            {
                ...
                TValue* kv = VM_KV(aux);
                LUAU_ASSERT(ttisstring(kv));
                if (ttistable(rb) && (hvalue(rb) == cl->env || hvalue(rb) == L->global->cb.tracesharedglobal))
                    luau_traceaccess(L, LUA_TRACE_SET_TABLE, hvalue(rb), kv, ra);
            ...
            VM_CASE(LOP_GETTABLE)
            {
                ...
                const bool traceEnvironment = ttistable(rb) && (hvalue(rb) == cl->env || hvalue(rb) == L->global->cb.tracesharedglobal);
                ...
                if (ttistable(rb) && ttisnumber(rc))
                {
                    ...
                    if (LUAU_LIKELY(unsigned(index) - 1 < unsigned(h->sizearray) && !h->metatable && double(index) == indexd))
                    {
                        ...
                        if (traceEnvironment)
                            luau_traceaccess(L, LUA_TRACE_GET_TABLE, h, rc, ra);
                        VM_NEXT();
                    }
                ...
                if (traceEnvironment)
                {
                    VM_CASE_STKID traceRb = VM_REG(LUAU_INSN_B(insn));
                    VM_CASE_STKID traceRc = VM_REG(LUAU_INSN_C(insn));
                    luau_traceaccess(L, LUA_TRACE_GET_TABLE, hvalue(traceRb), traceRc, VM_REG(LUAU_INSN_A(insn)));
                }
                VM_NEXT();
            }
            ...
            VM_CASE(LOP_SETTABLE)
            {
                ...
                VM_CASE_STKID rc = VM_REG(LUAU_INSN_C(insn));
                if (ttistable(rb) && (hvalue(rb) == cl->env || hvalue(rb) == L->global->cb.tracesharedglobal))
                    luau_traceaccess(L, LUA_TRACE_SET_TABLE, hvalue(rb), rc, ra);

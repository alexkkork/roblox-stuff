...
static int luaB_rawget(lua_State* L)
{
    ...
    access.kind = LUA_TRACE_RAW_GET;
    access.tablePointer = lua_topointer(L, 1);
    access.key.type = lua_type(L, 2);
    if (access.key.type == LUA_TSTRING)
        access.key.stringValue = lua_tolstring(L, 2, &access.key.stringLength);
    else if (access.key.type == LUA_TNUMBER)
        access.key.numberValue = lua_tonumber(L, 2);
    else if (access.key.type == LUA_TBOOLEAN)
        access.key.booleanValue = lua_toboolean(L, 2);
    else
        access.key.pointerValue = lua_topointer(L, 2);
    lua_settop(L, 2);
    lua_rawget(L, 1);
    if (lua_callbacks(L)->traceaccess)
    {
        access.value.type = lua_type(L, -1);
        if (access.value.type == LUA_TSTRING)
            access.value.stringValue = lua_tolstring(L, -1, &access.value.stringLength);
        else if (access.value.type == LUA_TNUMBER)
            access.value.numberValue = lua_tonumber(L, -1);
        else if (access.value.type == LUA_TBOOLEAN)
            access.value.booleanValue = lua_toboolean(L, -1);
        else
            access.value.pointerValue = lua_topointer(L, -1);
        lua_callbacks(L)->traceaccess(L, &access);
    }
    return 1;
}

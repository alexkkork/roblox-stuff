...
void store_be32(std::uint8_t* p, std::uint32_t value)
{
...
std::string default_name(const std::filesystem::path& path)
{
...
Sha256 sha256(std::span<const std::uint8_t> data)
{
    ...
    for (std::size_t offset = 0; offset < msg.size(); offset += 64)
    {
        ...
        for (std::size_t i = 0; i < 16; ++i)
            w[i] = load_be32(msg.data() + offset + i * 4);
        for (std::size_t i = 16; i < 64; ++i)
        {
            std::uint32_t s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >> 3u);
            std::uint32_t s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >> 10u);
...
Guard& Guard::add_file(std::filesystem::path path, std::string expected_sha256_hex, std::string name)
{
    FileRule rule;
    rule.path = std::move(path);
    rule.expected_sha256_hex = std::move(expected_sha256_hex);
    rule.name = name.empty() ? default_name(rule.path) : std::move(name);
    files_.push_back(std::move(rule));
    return *this;
}

Guard& Guard::add_self_file(std::string expected_sha256_hex, std::string name)
{
    std::filesystem::path path = self_path();
    return add_file(path, std::move(expected_sha256_hex), name.empty() ? "self" : std::move(name));
}

Guard& Guard::add_memory(std::string name, const void* data, std::size_t size, Sha256 expected_sha256)
{
    MemoryRule rule;
...
Guard& Guard::seal_memory(std::string name, const void* data, std::size_t size)
{
...
Guard& Guard::on_failure(FailureCallback callback)
{
    failure_ = std::move(callback);
    return *this;
}

CheckResult Guard::check() const
{
    CheckResult result;
    ...
    for (const FileRule& rule : files_)
    {
        ...
        try
        {
            ...
            if (!constant_time_equal(actual, hex(expected)))
            {
                result.ok = false;
                result.findings.push_back({FindingKind::FileHashMismatch, rule.name, hex(expected), actual, rule.path.string()});
            }
        }
...
bool Guard::check_or_fail() const
{
...
const std::vector<FileRule>& Guard::files() const
{
    return files_;
}

const std::vector<MemoryRule>& Guard::memory() const
{
    return memory_;
}

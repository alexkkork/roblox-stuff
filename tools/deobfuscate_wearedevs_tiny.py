...
def strip_decode_prelude(src: str) -> str:
    ...
    # Keep the transformed chunk on its original first line.  This family
    # fingerprints pcall error line numbers, so adding a newline here changes
    # control flow before the payload is reached.
    return "--[[ decoded constants inserted by tools/deobfuscate_wearedevs_tiny.py ]] " + compact

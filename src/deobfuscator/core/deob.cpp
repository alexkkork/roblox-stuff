...
json luraphRuntimeLaneValue(std::string_view encoded)
{
    ...
    if (encoded.starts_with("c:"))
    {
        ...
        const std::optional<uint64_t> objectId = separator == std::string_view::npos
            ? std::nullopt : parseTraceInteger<uint64_t>(encoded.substr(2, separator - 2));
        const std::string_view nameHex = separator == std::string_view::npos
            ? std::string_view{} : encoded.substr(separator + 1);
        const std::optional<std::string> name = decodeTraceHex(nameHex);
        if (objectId && *objectId > 0 && name)
            return provenance({{"type", "function"}, {"value", nullptr}, {"primitive", false}, {"callable", true},
                {"object_id", *objectId}, {"name", printableAscii(*name) ? json(*name) : json(nullptr)},
                {"name_hex", std::string(nameHex)}});
    }
    ...
    const LuraphRuntimeStructureTrace* structureSeed = nullptr)
{
    ...
    while (std::getline(input, line))
    {
        ...
        if (line.starts_with("@@LPH_ACT_ARG_OBJECT_V1@@\t"))
        {
            ...
            if (!activation || !prototype || !argumentIndex || !objectId || *activation == 0 ||
                *prototype == 0 || *argumentIndex == 0 || *argumentIndex > 16 || *objectId == 0 ||
    ...
    std::map<std::pair<uint64_t, size_t>, std::set<uint64_t>> observedClosureWriteObjects;
    ...
    for (const json& step : trace.steps)
    {
        ...
        for (const json& write : step["register_writes"])
        {
            ...
            if (write["value"].contains("object_id") && write["value"]["object_id"].is_number_unsigned())
                observedClosureWriteObjects[site].insert(write["value"]["object_id"].get<uint64_t>());
        }
    ...
    for (const auto& [site, topValues] : laneTopValues)
    {
        ...
        json functionObjectIds = json::array();
        if (const auto objects = observedClosureWriteObjects.find(closureSite);
            objects != observedClosureWriteObjects.end())
            for (uint64_t objectId : objects->second)
                if (objectId > 0)
                    functionObjectIds.push_back(objectId);
        trace.closure_descriptors[{prototype, pc}] = {
            ...
            {"function_object_ids", std::move(functionObjectIds)},
    ...
    const std::vector<json>& observations)
{
    const json range = handler.value("range", json::object());
    const std::string source = handler.value("candidate_source", "");
    constexpr std::string_view exactLoad = "L[S[_]]=(R[Z[_]]);";
    const auto laneInteger = [](const json& lanes, std::string_view name) -> std::optional<int64_t> {
        if (!lanes.is_object() || !lanes.contains(std::string(name)))
            return std::nullopt;
        return luraphObservedInteger(lanes[std::string(name)]);
    ...
    const std::optional<int64_t> capture = laneInteger(effectiveLanes, "Z");
    if (!range.is_object() || range.value("begin", size_t(0)) != 326066 ||
        range.value("end", size_t(0)) != 326084 || source != exactLoad || !destination ||
        !capture || *destination < 0 || *capture < 0 || observations.empty())
        return std::nullopt;
    ...
    for (const json& observation : observations)
    {
        ...
        const json writes = observation.value("register_writes", json::array());
        const json lanes = observation.value("runtime_lanes", json::object());
        if (observation.value("opcode", int64_t(-1)) != 136 ||
            laneInteger(lanes, "S") != destination || laneInteger(lanes, "Z") != capture ||
            !guardPath.is_object() || !guardPath.value("complete", false) || guardPath.value("overflow", true) ||
            observation.value("next_pc", std::numeric_limits<int64_t>::min()) !=
                static_cast<int64_t>(pc + 1) || !writes.is_array() || writes.size() != 1 ||
            !writes.front().is_object() ||
    ...
    return json{
        ...
        {"static_semantic", false},
        {"source_claim", false},
        {"proof", "locked_opcode136_capture_load_and_runtime_write_validated"},
        {"observation_count", observations.size()},
        ...
        {"runtime_validation", {
            {"validated_fields", json::array({
                "handler_body", "destination_register", "capture_index", "guard_path", "next_pc",
            })},
            {"destination_register", *destination},
...
std::optional<json> recognizeLuraphOpcode186LookupAndPreserve(
    uint64_t prototype,
    size_t pc,
    const json& handler,
    const json& effectiveLanes,
    const std::vector<json>& observations)
{
    const json range = handler.value("range", json::object());
    const std::string source = handler.value("candidate_source", "");
    constexpr std::string_view exactPreserve = "i=S[_];F=L[r[_]];L[i+0X001]=F;";
    constexpr std::string_view exactLookup = "(L)[i]=(F[V[_]]);";
    const auto laneInteger = [](const json& lanes, std::string_view name) -> std::optional<int64_t> {
        if (!lanes.is_object() || !lanes.contains(std::string(name)))
            return std::nullopt;
        return luraphObservedInteger(lanes[std::string(name)]);
    ...
    return json{
        ...
        {"static_semantic", false},
        {"source_claim", false},
        {"proof", "locked_opcode186_lookup_preserve_branch_and_runtime_origins_validated"},
        {"observation_count", observations.size()},
        {"operations", json::array({
            {
                {"kind", "register_write"},
                {"register", constant(*base + 1)},
                {"value", registerRead(*sourceRegister)},
            },
            {
                {"kind", "register_write"},
                {"register", constant(*base)},
        ...
        {"runtime_validation", {
            {"validated_fields", json::array({
                "handler_branch", "source_register", "preserved_register", "lookup_index",
                "write_origin", "written_types", "guard_path", "next_pc",
            })},
    ...
    const std::vector<json>& observations)
{
    const json range = handler.value("range", json::object());
    const std::string source = handler.value("candidate_source", "");
    constexpr std::string_view exactCall = "L[i]=L[i](L[i+0X1]);";
    constexpr std::string_view exactTop = "q=i;";
    const auto laneInteger = [](const json& lanes, std::string_view name) -> std::optional<int64_t> {
        if (!lanes.is_object() || !lanes.contains(std::string(name)))
            return std::nullopt;
        return luraphObservedInteger(lanes[std::string(name)]);
    ...
    const std::optional<int64_t> base = laneInteger(effectiveLanes, "r");
    if (!range.is_object() || range.value("begin", size_t(0)) != 351157 ||
        range.value("end", size_t(0)) != 351270 || source.find(exactCall) == std::string::npos ||
        source.find(exactTop) == std::string::npos || !base || *base < 0 ||
        *base == std::numeric_limits<int64_t>::max() || observations.empty())
        return std::nullopt;
    ...
    for (const json& observation : observations)
    {
        ...
        const json writes = observation.value("register_writes", json::array());
        const json lanes = observation.value("runtime_lanes", json::object());
        if (observation.value("opcode", int64_t(-1)) != 104 ||
            laneInteger(lanes, "r") != base || !guardPath.is_object() ||
    ...
    return json{
        ...
        {"static_semantic", false},
        {"source_claim", false},
        {"proof", "locked_opcode104_suffix_and_runtime_result_validated"},
        {"observation_count", observations.size()},
        {"operations", json::array({
            {
                {"kind", "register_write"},
        ...
        {"runtime_validation", {
            {"validated_fields", json::array({
                "handler_suffix", "argument_count", "destination_register",
                "guard_path", "next_pc",
            })},
            {"argument_count", 1},
            {"destination_register", *base},
    ...
    for (const auto& [id, prototype] : trace.prototypes)
    {
        ...
        for (const auto& [pc, instruction] : prototype.instructions)
        {
            ...
            if (!semanticAccepted && opcode == 212 && handler != handlers.end() &&
                ...
                childActivationsForSite != childActivationsBySite.end())
            {
                if (std::optional<json> recognized = recognizeLuraphOpcode212ZeroArgumentCall(
                        id, pc, handler->second, effectiveLanes, observedSite->second,
            ...
            if (!semanticAccepted && opcode == 136 && handler != handlers.end() &&
                ...
                observedSite != observationsBySite.end())
            {
                if (std::optional<json> recognized = recognizeLuraphOpcode136CaptureLoad(
                        id, pc, handler->second, effectiveLanes, observedSite->second))
                {
                    row["observational_semantic_operation"] = std::move(*recognized);
                    row["opcode136_capture_load_recognition"] = {
                        {"status", "runtime_validated"},
                        {"validated_observations", observedSite->second.size()},
                    };
                    ++observationalSemanticLifted;
                    observationalOperationCounts["capture_load"] =
                        observationalOperationCounts.value("capture_load", size_t(0)) + 1;
                }
                else
                    row["opcode136_capture_load_recognition"] = {
                        {"status", "evidence_mismatch"},
            ...
            if (!semanticAccepted && opcode == 186 && handler != handlers.end() &&
                row["observational_semantic_operation"].is_null() &&
                observedSite != observationsBySite.end())
            {
                if (std::optional<json> recognized = recognizeLuraphOpcode186LookupAndPreserve(
                        id, pc, handler->second, effectiveLanes, observedSite->second))
                {
                    row["observational_semantic_operation"] = std::move(*recognized);
                    row["opcode186_lookup_preserve_recognition"] = {
                        {"status", "runtime_validated"},
                        {"validated_observations", observedSite->second.size()},
                    };
                    ++observationalSemanticLifted;
                    observationalOperationCounts["lookup_and_preserve"] =
                        observationalOperationCounts.value("lookup_and_preserve", size_t(0)) + 1;
                }
                else
                    row["opcode186_lookup_preserve_recognition"] = {
                        {"status", "evidence_mismatch"},
            ...
            if (!semanticAccepted && opcode == 104 && handler != handlers.end() &&
                row["observational_semantic_operation"].is_null() &&
                observedSite != observationsBySite.end())
            {
                if (std::optional<json> recognized = recognizeLuraphOpcode104OneArgumentCall(
                        id, pc, handler->second, effectiveLanes, observedSite->second))
                {
                    row["observational_semantic_operation"] = std::move(*recognized);
                    row["opcode104_one_argument_call_recognition"] = {
                        {"status", "runtime_validated"},
                        {"validated_observations", observedSite->second.size()},
                    };
                    ++observationalSemanticLifted;
...
Result finishLuraphAnalysis(const Options& options, std::string_view source, const luraph::EnvelopeAnalysis& analysis, json report)
{
        ...
        (!dynamicTrace->calls.empty() || payloadRoot))
    {
        ...
        writeJson(semanticCandidateMapPath, {
            ...
            {"unobserved_instruction_boundaries", semanticCandidate->unobserved_instruction_boundaries},
            ...
            {"root_argument_capture_prototypes_hydrated", semanticCandidate->root_argument_capture_prototypes_hydrated},
            {"root_argument_capture_slots_hydrated", semanticCandidate->root_argument_capture_slots_hydrated},
